...
Corrigido para evitar 'Cannot run the event loop while another loop is running'
"""
import os
import re
import json
...
# Configuração de logging (assuming this is done once, e.g., in main app)
logger = logging.getLogger(__name__)
...
class ViralImageFinder:
    ...
    async def search_images(self, query: str) -> List[Dict]:
        ...
        all_results = []
        # --- Queries otimizadas para busca de conteúdo educacional ---
        queries = [
    ...
    async def _search_serper(self, query: str) -> List[Dict]:
        ...
        payload = json.dumps({
            "q": query,
            "num": 2, # Reduzido para evitar sobrecarga
            "safe": "off",
            "imgSize": "large",
            "imgType": "photo"
        })
        ...
        # Usar requests para chamadas API síncronas é ok, mas considerar aiohttp para totalmente async
    ...
    # --- Correção 1: Tornar extract_image_data async ---
    async def extract_image_data(self, image_url: str, post_url: str, platform: str) -> Optional[str]:
        ...
        try:
            ...
            # Usar aiohttp para requisição assíncrona
                ...
                async with session.get(image_url) as response:
                    ...
                    if 'image' in content_type:
                        ...
                        if content_length > 10 * 1024 * 1024:  # 10MB
                            logger.warning(f"Imagem muito grande ({content_length} bytes): {image_url}")
                            # Tentar tirar uma screenshot
                            screenshot_path = await self.take_screenshot(post_url, platform)
                        ...
                        # Se não for uma imagem, tente resolver a URL real
                        ...
                        if resolved_url and resolved_url != image_url:
                            ...
                            return await self.extract_image_data(resolved_url, post_url, platform)
                        else:
            ...
            logger.error(f"Erro de requisição aiohttp ao baixar imagem {image_url}: {e}")
            # Tentar tirar uma screenshot
            screenshot_path = await self.take_screenshot(post_url, platform)
            return screenshot_path
        except Exception as e:
    ...
    def _resolve_image_from_post(self, post_url: str, platform: str) -> Optional[str]:
        ...
        try:
            ...
            # Lista de seletores para meta tags de imagem
            selectors = [
            ...
            for selector in selectors:
                meta = soup.find('meta', attrs={'property': 'og:image'} if 'property' in selector else {'name': 'twitter:image'})
                if meta and meta.get('content'):
                    return meta['content']

            # Se não encontrou, tente extrair a URL diretamente do corpo da página
    ...
    # --- Correção 2: Tornar take_screenshot async e remover loop interno ---
    async def take_screenshot(self, post_url: str, platform: str) -> Optional[str]:
        ...
        # A própria função agora é async, então podemos usar await diretamente
        try:
            async with async_playwright() as p:
                # Escolher navegador
                if self.config['playwright_browser'] == 'firefox':
            ...
            # Garantir que os recursos sejam fechados mesmo em erro
            # (já é feito pelo async with, mas reforçando)
        ...
        for i, result in enumerate(search_results[:self.config['max_images']]):
            try:
                ...
                if self.config.get('extract_images', True):
                    ...
                    # --- Correção 3: Usar await para chamar a função async corrigida ---
                    image_path = await self.extract_image_data(image_url_from_result, page_url, platform)
...
# Função principal para uso do módulo (opcional, pode ser removida se usado como serviço)
# async def main():
